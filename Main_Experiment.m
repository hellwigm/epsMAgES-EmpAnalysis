...
% in the paper we used D=10 and D=100
D=10;
...
% algorithmically reduced variants
ESvariant{2}='epsMAES';		% 'epsMAgES' w/o gradient-based repair
ESvariant{3}='lexMAgES';	% 'epsMAgES' w/o epsilon-level constraint handling
ESvariant{4}='lexMAES';		% 'epsMAgES' w/o epsilon-level constraint handling and w/o gradient-based repair
ESvariant{5}='epsMAgESwo';      % 'epsMAgES' w/o back-calculation for tranformation matrix learning after repair
ESvariant{6}='epsMAgESnl';      % 'epsMAgES' w/o limitation of the mutation strength sigma
ESvariant{7}='epsSAgES';        % 'epsMAgES' w/o tranformation matrix control

    for jj=1:7    % for each ESvariant do
        ...
        for kk=1:length(funs)  
            ...
            save([foldername '/' strategy '_RS_on_fun' num2str(func_num) 'D' num2str(D) '.mat'],'GB','Dyn','FitT','problem','input','-v7')
