function [out,global_best, dyn]=epsMAgESnl(problem,input,CEC_fun_no)
    ...
    %
    % VARIATION: allow for unlimited mutation strength growth
 ...
 %% VARIATION
        ...
        sigma =  sigma * exp((input.cs/2)*(norm(ps)^2/input.dim - 1)); %   sigma = min(sigma * exp((input.cs/2)*(norm(ps)^2/input.dim - 1)),sigmax); 
