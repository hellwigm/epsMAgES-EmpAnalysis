function [out,global_best, dyn]=lexMAES(problem,input,CEC_fun_no)
    ...
    %
    % VARIATION: the epsilon constraint handling is replaced by the
    % lexicographic constraint handling approach --> initially set TC=0 !!!
    % AND omit the gradient-based repair step
...
%% VARIATION   TC = 0 and EPSILON = 0
    TC = 0; % TC=1000;
%     n=ceil(0.9*size(newpop.conv,2));   
%     index=lex_sort(newpop.f,newpop.conv);
    EPSILON=0; % mean(newpop.conv(index(1:n)));
    Epsilon= EPSILON;    
%     CP=max(3,(-5-log(EPSILON))/log(0.05));
    
    [ranking]           = eps_sort(newpop.f,newpop.conv,0);                          % epsilon Ranking
